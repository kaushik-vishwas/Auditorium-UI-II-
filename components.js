(function ($) {
  ...
  if (typeof window.theme === "undefined") {
    window.theme = {
      fonts: ["Raleway", "Cinzel"],
      ajax: { enabled: true, preventRules: "" },
      animations: {
        triggerHook: 0.85,
        timeScale: {
          onScrollReveal: 1,
      ...
      cursorFollower: { enabled: true, labels: { slider: "Drag" } },
      smoothScroll: {
        enabled: true,
        damping: 0.12,
        renderByPixels: true,
        continuousScrolling: false,
        plugins: { edgeEasing: true },
      },
      contactForm7: { customModals: true },
    };
  }
  ...
  window.$window.on("load", function () {
    ...
    fontObserver()
      .then(() =>
        SetText.splitText({ target: window.$document.find(".js-split-text") })
      )
      .then(() =>
        SetText.setLines({
      ...
      .then(() => {
        initComponentsOnce({ scope: window.$document });
        initComponents({ scope: window.$document });
      })
      ...
      .then(() => {
        if (
          ...
          new Cursor({
            ...
            highlightElements:
              "a:not(a[data-arts-cursor]):not(.social__item a):not(.section-video__link):not(.no-highlight), button:not(button[data-arts-cursor]), .filter__item, .section-nav-projects__header",
            highlightScale: 1.5,
    ...
    if (window.theme.ajax.enabled) {
      new PJAX({ target: window.$barbaWrapper, scope: window.$document });
    }
  });
  function initComponents({
    ...
    new AsideCounters({ target: scope.find(".aside-counters"), scope });
    new Arrow({ target: scope.find(".js-arrow") });
    new SectionMasthead({ target: scope.find(".section-masthead"), scope });
    new SectionContent({ target: scope.find(".section-content"), scope });
    new SectionProjectsSlider({
      target: scope.find(".section-projects-slider"),
      scope,
    });
    new SectionList({ target: scope.find(".section-list"), scope });
    new ChangeTextHover({
      target: scope.find(
    ...
    new PSWPGallery({
      ...
      scope,
      options: {
        history: window.theme.ajax.enabled ? false : true,
    ...
    new GMap({ target: scope.find(".js-gmap"), scope });
    new Form({ target: scope, scope });
    new FormAJAX({ target: scope.find(".js-ajax-form"), scope });
    new FormAJAX2({ target: scope.find(".js-ajax-form2"), scope });
    ...
    new SectionGrid({ target: scope.find(".section-grid"), scope });
    ...
    new SectionScroll({ target: scope.find(".section-scroll"), scope });
    ...
    _setCurtain() {
      gsap.registerEffect({
        ...
        effect: (target, config) => {
          ...
          tl.set($target, { display: "none", autoAlpha: 1, y: config.y }).set(
            $svg,
    ...
    _moveCurtain() {
      gsap.registerEffect({
        ...
        effect: (target, config) => {
          ...
          tl.set($target, { display: "block", autoAlpha: 1 })
            .set([$curveTop, $curveBottom], { visibility: "hidden" })
            .to($target, { y: config.y, duration: 1.8, ease: "expo.inOut" });
          if (config.curve === "top") {
            tl.set($normal, { visibility: "visible" }, "0")
              .to(
                $normal,
                { duration: 0.9, ease: "power2.out", morphSVG: $curveTop[0] },
                "-=1.8"
              )
              .to(
                $normal,
        ...
        extendTimeline: true,
        defaults: { duration: 2.4, curve: "top", y: "0%" },
      });
    }
    _revealCurtain() {
      gsap.registerEffect({
        ...
        effect: (target, config) => {
          ...
          const $normal = $target.find(".curtain-svg__normal"),
            $curve = $target.find(".curtain-svg__curve");
          tl.set($target, { y: "100%", autoAlpha: 1 })
            .set($normal, { visibility: "visible" })
            .set($curve, { visibility: "hidden" })
            .to($target, { y: "0%", duration: 1.8, ease: "expo.inOut" })
            .to(
              $normal,
              { duration: 0.9, ease: "power2.out", morphSVG: $curve[0] },
              "-=1.8"
            )
            .to(
              $normal,
    ...
    _animateChars() {
      gsap.registerEffect({
        ...
        defaults: {
          ...
          duration: 1,
          ease: "power3.inOut",
          stagger: distributeByPosition({ from: "start", amount: 0.3 }),
        },
        extendTimeline: true,
      });
    }
    ...
    _hideChars() {
      gsap.registerEffect({
        ...
        defaults: {
          ...
          autoAlpha: 0,
          ease: "power3.inOut",
          stagger: distributeByPosition({ from: "center", amount: 0.3 }),
        },
        extendTimeline: true,
      });
    }
  ...
  class BaseGLAnimation {
    ...
    _loadTextures() {
      ...
      this.items.each(function (index) {
        ...
        promises.push(self._loadTexture({ loader: self.loader, url, index }));
      });
  ...
  class Pswp extends BaseComponent {
    constructor({ scope, target, options }) {
      super({ scope, target });
      this.options = options || { history: false, showAnimationDuration: 300 };
    ...
    _openPhotoSwipe({
      ...
      const items = this._getItems(galleryElement, index),
        options = { galleryUID: galleryElement.attr("data-pswp-uid") };
    ...
    _getMediaTypeFromURL(url, size, autoplay = false) {
      const result = { type: false, html: null },
        iframeSize = size ? size.split("x") : [640, 360],
        attr = { video: autoplay ? "muted playsinline loop autoplay" : "" },
        param = {
    ...
    _getItems($galleryElement, activeIndex = 0) {
      ...
      $items.each(function (index) {
        const $el = $(this),
          ...
          autoplay = $el.attr("data-autoplay") && activeIndex === index,
          src = $el.attr("href"),
  ...
  class Scroll {
    ...
    static restoreScrollTop() {
      if (typeof window.SB !== "undefined") {
        ...
        $("html, body").animate({ scrollTop: window.lastTop });
      }
    }
    static lock(lock = true) {
      ...
      if (lock === true) {
        if (typeof window.SB !== "undefined") {
          window.SB.updatePluginOptions("lockscroll", { lock: true });
        }
      ...
      if (lock === false) {
        ...
        if (typeof window.SB !== "undefined") {
          window.SB.updatePluginOptions("lockscroll", { lock: false });
        }
      }
    }
  ...
  class ScrollAnimation extends BaseComponent {
    ...
    _createScene({
      ...
      const masterTL = new gsap.timeline({ delay: delay });
      let $trigger = element,
        scale = 1;
  ...
  class SetText {
      ...
      return new Promise((resolve) => {
        ...
        target.each(function () {
          ...
          if (distribute === true) {
            switch (textAlign) {
              case "left":
                instance._setFromLeft({ lines: $lines, x, y });
                break;
              case "center":
                instance._setFromCenter({ lines: $lines, x, y });
                break;
              case "right":
                instance._setFromRight({ lines: $lines, x, y });
                break;
            }
          } else {
            instance._setFromLeft({ lines: $lines, x, y });
    ...
    _setFromCenter({ lines, x, y }) {
      ...
      lines.each(function () {
        ...
        if ($wordsInCurrentLine.length === 1) {
          self._setCharsSingleWord({ words: $wordsInCurrentLine, x, y });
        }
        if (
          $wordsInCurrentLine.length !== 1 &&
          $wordsInCurrentLine.length % 2 === 0
        ) {
          self._setCharsEvenWords({ words: $wordsInCurrentLine, x, y });
        }
        if (
          $wordsInCurrentLine.length !== 1 &&
          $wordsInCurrentLine.length % 2 !== 0
        ) {
          self._setCharsOddWords({ words: $wordsInCurrentLine, x, y });
        }
      });
    }
    _setCharsSingleWord({ words, x, y }) {
      ...
      gsap.set($fistHalfWord, { x: -x, y: -y, autoAlpha: 0 });
      gsap.set($secondHalfWord, { x, y, autoAlpha: 0 });
    }
    _setCharsOddWords({ words, x, y }) {
      ...
      $secondHalf.each(function () {
        const $charsInWord = $(this).find(".split-text__char");
        gsap.set($charsInWord, { x, y, autoAlpha: 0 });
      });
  ...
  class Slider extends BaseComponent {
    ...
    _getSliderDots({ slider, container }) {
      return new SliderDots({ slider, container });
    }
    _getSliderCounter({ slider, counter = { current, total, style, zeros } }) {
  ...
  function PJAXAnimateClonnedImage(data, duration = 2.0) {
    ...
    return new Promise((resolve) => {
      ...
      tl.setCurtain($curtain, { background })
        .set($clone, { maxWidth: "100%", maxHeight: "100%" })
        .add([
          gsap.to(bgClone, {
            paddingBottom: 0,
          ...
          gsap.effects.moveCurtain($curtain, { y: "0%", duration: 1.2 }),
          gsap.to($clone, {
        ...
        .to(
          $nextContainer,
          { duration: 0.2, clearProps: "all", autoAlpha: 1 },
          "-=0.3"
        )
        .setCurtain($curtain)
        .add(() => {
  ...
  function PJAXFinishLoading(data) {
    return new Promise((resolve) => {
      ...
      if (typeof window.$spinner !== "undefined" && window.$spinner.length) {
        gsap.to(window.$spinner, 0.6, { autoAlpha: 0 });
      }
    ...
    return new Promise((resolve) => {
      ...
      PJAXUpdateAudioBackground(data).then(() => {
        Promise.all([
          ...
          .then(() =>
            SetText.splitText({ target: $nextContainer.find(".js-split-text") })
          )
          .then(() =>
            SetText.setLines({
  ...
  function PJAXStartLoading(data) {
    return new Promise((resolve) => {
      ...
      if (typeof window.$spinner !== "undefined" && window.$spinner.length) {
        gsap.to(window.$spinner, 0.6, { autoAlpha: 1 });
      }
      resolve(true);
    });
  }
  ...
  const PJAXTransitionFullscreenSlider = {
    ...
    beforeLeave: (data) => {
      return new Promise((resolve) => {
        ...
        if (imageSrc) {
          tl.set($bg, { backgroundImage: `url(${imageSrc})` }).add(() => {
            PJAXCloneImage($image).then(() => {
              gsap.to($content, {
                duration: 0.3,
                autoAlpha: 0,
    ...
    beforeEnter: (data) => {
      return new Promise((resolve) => {
        const tl = new gsap.timeline(),
          ...
          background = $nextMasthead.attr("data-background-color");
        tl.setCurtain($curtain, { background })
          .moveCurtain($curtain, { y: "0%", duration: 1.2 })
          .setCurtain($curtain)
          .set($nextContainer, { clearProps: "all", autoAlpha: 1 })
          .add(() => {
            resolve(true);
          });
      });
  ...
  const PJAXTransitionOverlayMenu = {
    ...
    afterEnter: (data) => {
      return new Promise((resolve) => {
        ...
        tl.set([$nextContainer, $currentContainer], { autoAlpha: 0 })
          .setCurtain($curtain, { background })
          .moveCurtain($curtain, { duration: 1.2, y: "0%", curve: "top" })
          .add(closeTl, "-=0.8")
          .setCurtain($curtain)
          .set($nextContainer, { clearProps: "all", autoAlpha: 1 })
          .add(() => {
  ...
  function PJAXUpdateTrackers() {
    ...
    function updateYaMetrika() {
      if (typeof ym === "function") {
        ...
        ym(trackingID, "hit", window.location.href, { title: document.title });
      }
    }
  ...
  class PJAX extends BaseComponent {
    ...
    run() {
      barba.init({
        sync: true,
        timeout: 10000,
        cacheIgnore: window.Modernizr.touchevents ? true : false,
        prevent: ({ el }) => {
          const $el = $(el),
            url = $el.attr("href"),
            exludeRules = [
  ...
  class AsideCounters extends BaseComponent {
    ...
    run() {
      ...
      $counters.each(function () {
        new Counter({ scope: self.scope, target: $(this) });
      });
    }
  }
  class Arrow extends BaseComponent {
    ...
    set() {
      ...
      gsap.set(this.$circles, { clearProps: "all" });
  ...
  class ChangeTextHover extends BaseComponent {
    constructor({ target, scope, pageIndicator, triggers, options }) {
      super({ target, scope });
      this.options = options || { duration: 0.4, ease: "power4.out" };
    ...
    _getTimelineShowHover({ normal, hover, line }) {
      return new gsap.timeline({ delay: 0.02 })
        .hideLines(hover, { y: "100%", duration: 0, stagger: 0 })
        .add([
    ...
    _getTimelineHideHover({ normal, hover, line }) {
      return new gsap.timeline({ delay: 0.02 })
        .hideLines(normal, { y: "100%", duration: 0, stagger: 0 })
        .add([
  ...
  class Counter extends ScrollAnimation {
    constructor(options) {
      ...
      this.counter = { val: this.numberStart };
      this.prepare();
      this.animate();
    }
    prepare() {
    ...
    animate() {
      ...
      this._createScene({ element: this.$target, timeline: tl });
    }
    _addZeros(value) {
  ...
  class Cursor extends BaseComponent {
    ...
    set() {
      gsap.set(this.$target, { display: "block" });
      gsap.to(this.$target, {
        duration: 0.6,
        scale: 1,
        autoAlpha: 1,
    ...
    _bindEvents() {
      ...
      this.$scope
        ...
        .on("mouseenter", ".slider__dot", () => {
          this.setCursor({ hide: true });
        })
        .on("mouseleave", ".slider__dot", (e) => {
          this.setCursor({ hide: false });
          this.scale = 1;
          this._scaleCursor();
        ...
        .on("mouseenter", self.cursorElements, (e) => {
          ...
          this._setLabel({ label: $target.data("arts-cursor-label") || "" });
          this._setIcon({ icon: $target.data("arts-cursor-icon") || "" });
          this._hideArrows();
        ...
        .on("mouseleave", self.cursorElements, () => {
          this.setCursor({ hide: false });
          this._setLabel({ label: "", hide: true });
          this._setIcon({ icon: "", hide: true });
          this.scale = 1;
          this._scaleCursor();
        })
        ...
        .on("arts/slider/touchDown", (e) => {
          this.setCursor({ hide: true });
          this._setLabel({ label: this.labels.slider });
          this._setIcon({ icon: "", hide: true });
        ...
        .on("arts/slider/touchUp", () => {
          this.setCursor({ hide: false });
          this._setLabel({ label: this.labels.slider, hide: true });
          this._setIcon({ icon: "", hide: true });
          this._hideArrows();
          this.scale = 1;
          this._scaleCursor();
    ...
    _hideArrows() {
      this.timelineArrows
        ...
        .to(
          ...
          { duration: this.animDuration, autoAlpha: 0, x: 0, y: 0 }
        );
    }
    ...
    _resetMagnifiedElement(element) {
      ...
      if (element && element.length) {
        gsap.to(element, { duration: 0.4, y: 0, x: 0 });
      }
    }
  ...
  class EffectDistortion extends BaseGLAnimation {
    constructor({ slider, canvas, aspect = 1.5, displacementImage, items }) {
      ...
      this.uniforms = {
        ...
        texture: { type: "t", value: this.items[0].texture },
        texture2: { type: "t", value: this.items[1].texture },
  ...
  class EffectStretch extends BaseGLAnimation {
    constructor({ target, canvas, items, options }) {
      ...
      this.uniforms = {
        ...
        uAlpha: { value: 0 },
        uScale: { value: Math.abs(this.options.scaleTexture - 2) },
      };
      this.geometry = this._getPlaneBufferGeometry();
      this.material = this._getShaderMaterial();
      this.plane = this._getPlane({
  ...
  class Filter {
    ...
    updateLinePosition($el) {
      ...
      if (!$el || !$el.length) {
        gsap.to(this.$line, { duration: 0.6, width: 0, ease: "expo.out" });
      } else {
  ...
  class GMap extends BaseComponent {
    ...
    _createMap() {
      ...
      if (this.styles) {
        $.extend(argsMap, { styles: this.styles });
      }
    ...
    _createMarker($marker) {
      ...
      if (MARKER_IMG) {
        $.extend(argsMarker, { icon: { url: MARKER_IMG } });
      }
      ...
      this._createInfoWindow({ marker, content: MARKER_CONTENT });
    }
  ...
  class Grid extends BaseComponent {
    constructor({ target, scope }) {
      super({ target, scope });
      this.lazyInstance = new LazyLoad({ scope });
  ...
  class Header {
    ...
    setMenu() {
      if (this.$overlay.length) {
        gsap.set(this.$overlay, { autoAlpha: 0, display: "none" });
      }
      if (this.$submenu.length) {
        gsap.set(this.$submenu, { autoAlpha: 0 });
      }
      if (this.$submenuButton.length) {
        gsap.set(this.$submenuButton, { autoAlpha: 0 });
      }
    ...
    openMenu() {
      return this.timeline
        ...
        .set(this.$overlay, { autoAlpha: 1, display: "flex" })
        .add([
          () => {
        ...
        .set(this.$adminBar, { position: "fixed" })
        .to(
          this.$headerLeft,
          { duration: 1.2, x: 30, autoAlpha: 0, ease: "expo.inOut" },
          "start"
        )
        .moveCurtain(this.$curtain, { duration: 1.2, y: "0%" }, "start")
        .add(() => {
          this.$header.addClass(this.openClass);
        }, "-=0.6")
        .add(
    ...
    closeMenu(force = false, cb) {
      ...
      return this.timeline
        ...
        .to(
          this.$headerLeft,
          { duration: 1.2, x: 0, autoAlpha: 1, ease: "expo.inOut" },
          "start"
        )
        .to(
          this.$submenuButton,
          { x: -10, autoAlpha: 0, duration: 0.3, ease: "expo.inOut" },
          "start"
        )
        .moveCurtain(
          this.$curtain,
          { duration: 1.2, y: "-100%", curve: "bottom" },
          "start"
        )
        .add(() => {
          this.$header.removeClass(this.openClass);
        ...
        .add(
          gsap.effects.hideLines(
            ...
            { stagger: { amount: 0, from: "end" }, y: "100%", duration: 0.6 }
          ),
          "start"
        )
        .add(() => {
    ...
    _bindEvents() {
      ...
      if (this.$submenuOpeners.length) {
        this.$submenuOpeners.on("click", function (e) {
          ...
          self._openSubmenu({ submenu: $submenu, currentMenu: $currentMenu });
        });
      }
      if (this.$submenuButton.length) {
        this.$submenuButton.on("click", (e) => {
          ...
          self._closeSubmenu({ submenu: $submenu, currentMenu: $prevMenu });
        });
      }
      window.$window
        .on("arts/preloader/end", () => {
    ...
    _correctTopOffset() {
      ...
      gsap.to(this.$header, { duration: 0.6, top });
    }
    _stick() {
      if (!this.$stickyHeader.length) {
        return;
      }
      this.stickyScene = new $.ScrollMagic.Scene({ offset: "1px" })
        .setClassToggle(
          this.$stickyHeader,
      ...
      this.timeline
        ...
        .add(() => {
          ...
          if (this.$submenu.hasClass(this.openClass)) {
            gsap.to(this.$submenuButton, { autoAlpha: 1, x: 0, duration: 0.3 });
    ...
    _closeSubmenu({ submenu, currentMenu }) {
      ...
      this.timeline
        ...
        .add(
          gsap.effects.animateLines($currentLinks, { y: "100%", duration: 0 }),
          "start"
        )
        .add(
  ...
  class LazyLoad {
    ...
    setPaddingBottom() {
      this.$images.each(function () {
        const $el = $(this),
          ...
          elPB = parseFloat((elHeight / elWidth) * 100);
        if (!elWidth || !elHeight) {
          return;
        }
    ...
    run() {
      this.loadImages({ target: this.$images });
      this.loadBackgrounds({ target: this.$backgrounds });
    }
      ...
      if (target && target.length) {
        const instance = target.Lazy({
          ...
          afterLoad: (el) => {
            const $el = $(el),
              $elParent = $el.parent();
            $el.imagesLoaded({ background: true }).always(() => {
  ...
  class Magnetic extends BaseComponent {
    ...
    _magnifyElement({ element, event, distance }) {
      ...
      if (targetDistance < distance) {
        gsap.to(element, { duration: 0.3, y: deltaY, x: deltaX });
      } else {
        gsap.to(element, { duration: 0.3, y: 0, x: 0 });
      }
    }
  ...
  class ArtsParallax {
    ...
    _getParallaxTimeline({ element, fromY, fromX, toY, toX, scale }) {
      return new gsap.timeline().fromTo(
        ...
        { y: toY || 0, x: toX || 0, force3D: true, ease: "linear.none" }
      );
    }
  ...
  function Preloader({
    ...
    cursor = { element: $("#js-cursor"), offset: { top: 0.0, left: 0.0 } },
    counter = {
      easing: "power4.out",
      duration: 25,
      start: 0,
    ...
    this.tweens = {
      drawCircle: gsap.fromTo(
        ...
        { rotate: 90, drawSVG: "100% 100%", transformOrigin: "center center" },
        {
          drawSVG: "0% 100%",
          rotate: 0,
          transformOrigin: "center center",
    ...
    this.start = () => {
      ...
      if (this.curtain.element.length) {
        gsap.set(this.curtain.svg, { fill: this.curtain.background });
        gsap.set(this.curtain.rect, { background: this.curtain.background });
        gsap.set(window.$pageContent, { autoAlpha: 0 });
      }
      this.timeline.add([
    ...
    this.finish = () => {
      return new Promise((resolve, reject) => {
        ...
        this.timeline
          .clear()
          .set(this.cursor.follower.outer, { attr: { transform: "" } })
          .to(
            this.cursor.follower.outer,
            {
              drawSVG: "0% 100%",
          ...
          .add(
            [
              gsap.effects.moveCurtain(this.curtain.element, { duration: 1.2 }),
          ...
          .set(window.$pageContent, { autoAlpha: 1 })
          .to(this.curtain.element, { autoAlpha: 0, delay: 0.4, duration: 0.3 })
  ...
  class PSWPAlbum extends Pswp {
    ...
    _bindClickAlbumLinks($gallery) {
      $gallery.on("click", (e) => {
        e.preventDefault();
        this._openPhotoSwipe({ index: 0, galleryElement: $gallery });
      });
    }
    _getItems($galleryElement, activeIndex = 0) {
      const self = this,
    ...
    _bindClickGalleryLinks($gallery) {
      ...
      $links.on("click", function (e) {
        ...
        tl.add(() => {
          ...
          .set(self.$container, { transition: "none" })
          .to($parallaxEl, { scale: 1, duration: 0.3 })
          .add(() => {
            self._openPhotoSwipe({ index, galleryElement: $gallery });
          })
          .set($parallaxEl, { delay: 0.1, scale: scale, overwrite: "all" })
          .set($el, { autoAlpha: 1 })
          .add(() => {
  ...
  class SmoothScroll {
    ...
    run() {
      if (
        ...
        (window.Modernizr.touchevents &&
          !this.$container.hasClass("js-smooth-scroll_enable-mobile")) ||
        window.Modernizr.touchevents
      ) {
        return false;
      }
    ...
    _correctAbsolutePositionElements() {
      ...
      gsap.to(this.$absoluteElements, { y: 0, duration: 0.3 });
      this.$absoluteElements.each(function () {
        const $el = $(this);
        window.SB.addListener((scrollbar) => {
          gsap.set($el, { y: -scrollbar.offset.y + barHeight });
        });
      });
    }
    _correctFixedPositionElements() {
      const barHeight =
        (this.$WPadminBar.length && this.$WPadminBar.height()) || 0;
      gsap.to(this.$fixedElements, { y: 0, duration: 0.3 });
  ...
  class ScrollDown extends BaseComponent {
    ...
    _scrollDown() {
      Scroll.scrollTo({ x: 0, y: window.innerHeight, duration: this.duration });
    }
  }
  class SectionContent extends ScrollAnimation {
    ...
    set() {
      ...
      gsap.set(this.$headline, { scaleX: 0 });
      gsap.set(this.$button, { y: 30, autoAlpha: 0 });
      ...
      gsap.set(this.$socialItems, { y: 30, autoAlpha: 0 });
    }
    run() {
      const tl = new gsap.timeline();
      tl.animateWords(
        this.$el,
        { ease: "power3.out", duration: 1.2, stagger: 0.04 },
        "start"
      )
        .to(
          this.$socialItems,
          { y: 0, autoAlpha: 1, stagger: 0.05, duration: 0.6 },
          "<0.2"
        )
        .animateLines(
          this.$el,
        ...
        .to(this.$button, { duration: 0.6, y: 0, autoAlpha: 1 }, "<0.2");
      this._createScene({
        element: this.$el,
        timeline: tl,
  ...
  class SectionGrid extends ScrollAnimation {
    ...
    set() {
      ...
      gsap.set(this.$filterUnderline, { autoAlpha: 1 });
    ...
    _bindGridFilter() {
      ...
      if (this.$filter.length) {
        ...
        this.filter.$items.on("click", function (e) {
          ...
          e.preventDefault();
          self.grid.isotope({ filter: filterBy });
        });
    ...
    _createFilter() {
      return new Filter({ scope: this.$scope, target: this.$filter });
    }
    _createGrid() {
      return new Grid({ target: this.$grid });
    }
  }
  class SectionList extends ScrollAnimation {
        ...
        $current
          ...
          .on("mouseleave touchend", () => {
            gsap.to(
              ...
              { ease: "power4.out", duration: 1.2, height: 0, x: "0%", y: "0%" }
            );
          });
      });
    }
    ...
    _animate() {
      const self = this;
      self._createScene({ element: this.$el });
      this.$listItems.each(function () {
        ...
        tl.to($thumb, { opacity: 1, scale: 1, duration: 1.2 }).add(
          [
            gsap.effects.animateWords($heading, { duration: 0.9 }),
        ...
        self._createScene({ element: $el, timeline: tl, reveal: false });
      });
    }
  }
  class SectionNavProjects extends ScrollAnimation {
      ...
      $(this.$circleButton)
        ...
        .on("click", (e) => {
          if (window.theme.ajax.enabled) {
            ...
            Scroll.scrollTo({ x: 0, y: offset, duration: 1200 });
          } else {
    ...
    _getSceneTimeline() {
      ...
      tl.to(
        ...
        .fromTo(
          this.$header,
          { pointerEvents: "initial", autoAlpha: 1, y: 0 },
          {
            pointerEvents: "none",
            duration: 0.75,
            autoAlpha: 0,
        ...
        .fromTo(
          this.$nextImage,
          { ease: "linear.none", autoAlpha: 0.1 },
          {
            autoAlpha: 1,
            duration: 1,
        ...
        .fromTo(
          this.$arcWrapper,
          { rotation: 0, transformOrigin: "center center" },
          { duration: 1, rotation: 720 },
          "start"
        )
        .to(
          this.$wrapperScroll,
          { y: -200, autoAlpha: 0, duration: 1 },
          "start"
        );
      return tl;
    }
  ...
  class SectionMasthead extends ScrollAnimation {
    ...
    _fixedMasthead() {
      ...
      if ($fixedMasthead.length) {
        const tl = new gsap.timeline();
        tl.fromTo($fixedMasthead, { autoAlpha: 1 }, { autoAlpha: 0 });
        new $.ScrollMagic.Scene({
          ...
          .setTween(tl)
          .setPin($fixedMasthead, { pushFollowers: false })
          .addTo(window.SMController);
      }
    }
    setAnimation() {
      ...
      gsap.set(this.$itemsMeta, { y: "100%", autoAlpha: 0 });
      gsap.set(this.$bg, { scaleY: 0, transformOrigin: "bottom center" });
      gsap.set(this.$overlay, { autoAlpha: 0 });
      gsap.set(this.$headline, { scaleX: 0 });
      gsap.set(this.$wrapperbutton, { y: 30, autoAlpha: 0 });
      gsap.set(this.$wrapperSD, { y: 30, autoAlpha: 0 });
    ...
    animate() {
      ...
      tl.to(
        this.$itemsMeta,
        { stagger: 0.2, duration: 0.6, autoAlpha: 1, y: "0%" },
        "start"
      );
      if (this.$bg.length && this.$background.length) {
        tl.to(
          this.$bg,
          { duration: 1.2, scaleY: 1, ease: "expo.inOut" },
          "start"
        ).to(
          this.$background,
          { duration: 2.4, autoAlpha: 1, scale: 1 },
          "-=0.4"
        );
      }
      if (this.$bg.length && !this.$background.length) {
      ...
      if (this.$overlay.length) {
        tl.to(this.$overlay, { autoAlpha: 1, duration: 1.2 }, "<0.2");
      }
      tl.animateChars(
        $target,
        {
      ...
      tl.animateWords($target, { ease: "power3.out", duration: 1.2 }, "<0.2")
        .animateLines(
          $target,
          { ease: "power3.out", duration: 1.2, stagger: 0.06 },
          "<0.2"
        )
        .to(this.$wrapperbutton, { duration: 0.6, y: 0, autoAlpha: 1 }, "<0.2")
        .to(this.$wrapperSD, { duration: 0.6, y: 0, autoAlpha: 1 }, "<0.2")
        .animateHeadline(this.$headline, { duration: 0.6 }, "<0.2");
      this._createScene({ element: $target, timeline: tl });
    }
  }
  class SectionProjectsSlider extends ScrollAnimation {
    ...
    set() {
      ...
      gsap.set(this.$footer, { autoAlpha: 0, y: "100%" });
      gsap.set(this.$arrowLeft, { x: -50, autoAlpha: 0 });
      gsap.set(this.$arrowRight, { x: 50, autoAlpha: 0 });
      gsap.set(this.$counter, { autoAlpha: 0 });
      gsap.effects.hideChars(this.$activeHeading, { x: 50, y: 0, duration: 0 });
      ...
      gsap.set(this.$activeButton, { y: 50, autoAlpha: 0 });
    ...
    run() {
      ...
      tl.add([
        ...
        .to(
          this.$activeButton,
          { duration: 1.2, ease: "power3.out", y: 0, autoAlpha: 1 },
          "-=1.2"
        )
        .add(
          [
            gsap.to(this.$footer, { duration: 1.2, autoAlpha: 1, y: "0%" }),
  ...
  class SectionTestimonials extends ScrollAnimation {
    ...
    set() {
      ...
      gsap.set(this.$activeSign, { y: 50, autoAlpha: 0 });
      gsap.effects.hideLines(this.$activeDescription, {
        y: "100%",
        duration: 0,
      });
  ...
  class SliderCategories extends BaseComponent {
    ...
    _getTimelineShowButton() {
      return new gsap.timeline()
        .hideLines(this.$button, { y: "100%", duration: 0, stagger: 0 })
        .add([
    ...
    _getTimelineShowItem($activeItem) {
      return new gsap.timeline()
        .hideLines($activeItem, { y: "100%", duration: 0, stagger: 0 })
        .add([
  ...
  class SliderDistortionEffect extends BaseComponent {
    ...
    _bindEvents() {
      this.slider.on("slideChange", () => {
        ...
        this.timeline
          ...
          .to(this.$canvasWrapper, { duration: this.speed * 2, scale: 1 });
      });
    }
    _isThreeLoaded() {
      return typeof window.THREE === "object";
    ...
    _hideSlider() {
      if (typeof window.elementor !== "undefined") {
        gsap.set(this.$wrapper, { display: "none" });
      } else {
  ...
  class SliderDots {
    ...
    _setCurrentDot(index = 0) {
      ...
      this.timeline
        ...
        .set($otherCircles, { strokeOpacity: 0 })
        .fromTo(
          $currentCircle,
          {
            strokeOpacity: 1,
    ...
    _getDotTemplate() {
      return `<svg viewBox="0 0 152 152"version="1.1"xmlns="http://www.w3.org/2000/svg"xmlns:xlink="http://www.w3.org/1999/xlink"><g fill="none"fill-rule="evenodd"><g transform="translate(-134.000000, -98.000000)"><path class="circle"d="M135,174a75,75 0 1,0 150,0a75,75 0 1,0 -150,0"></path></g></svg>`;
    }
  }
  ...
  class SliderTextTransitions {
    ...
    _bindEvents() {
      this.slider.on("slideChange", () => {
        if (this.slider.realIndex > this.slider.previousIndex) {
          this._slideChangeTransition({ direction: "next" });
        }
        if (this.slider.realIndex < this.slider.previousIndex) {
          this._slideChangeTransition({ direction: "prev" });
        }
      });
    }
    _initialSet() {
    ...
    _slideChangeTransition({ direction = "next" }) {
      const self = this,
        directions = this._getSlideAnimationDirections({ direction }),
      ...
      if ($activeLink.length) {
        self.timeline.to(
          $activeLink,
          { duration: self.speed, y: 0, autoAlpha: 1, ease: self.ease },
          `elementsSet-=${this.speed / 2}`
        );
      }
    ...
    _getSlideAnimationDirections({ direction = "next" }) {
      const directions = {
        in: { x: 0, y: 0, from: "start" },
        out: { x: 0, y: 0, from: "start" },
      };
      if (direction === "next") {
    ...
    _getAnimationDirections() {
      ...
      const directions = {
        offset: {
          x: { next: { in: 0, out: 0 }, prev: { in: 0, out: 0 } },
          y: { next: { in: 0, out: 0 }, prev: { in: 0, out: 0 } },
        },
        from: {
  ...
  class SliderImages extends Slider {
    ...
    run() {
      ...
      if (this.slider.params.autoplay.enabled) {
        this._pauseAutoplay({ slider: this.slider });
      }
      this._bindEvents();
    }
    _bindEvents() {
      ...
      this.$el.imagesLoaded().progress({ background: true }, (e) => {
        setTimeout(() => {
          this.slider.update();
        }, 300);
      });
  ...
  class SliderFullscreenProjects extends Slider {
    ...
    run() {
      ...
      if (this.sliderImg.params.autoplay.enabled) {
        this._pauseAutoplay({ slider: this.sliderImg });
      }
    ...
    _getSliderImages() {
      ...
      return new Swiper(this.$sliderImg, {
        ...
        controller: { control: [], by: "container" },
      });
    }
    _getSliderContent() {
  ...
  class SliderTestimonials extends Slider {
    ...
    _bindEvents() {
      ...
      this.$target.imagesLoaded().progress({ background: true }, (e) => {
        setTimeout(() => {
          this.slider.update();
        }, 300);
      });
  ...
  function runOnHighPerformanceGPU() {
    ...
    if (typeof webGLCanvas !== "undefined" && webGLCanvas !== null) {
      webGLCanvas.getContext("webgl", { powerPreference: "high-performance" });
    }
  }
